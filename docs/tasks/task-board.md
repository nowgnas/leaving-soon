...
| T-036 | Separate search results and add icon seat-space selector | refactor | P1 | DONE | Codex | `/client`, `/docs/logs/2026-05.md` | Visitor search no longer reads like active-report discovery, and reporter seat-space options use icon radio buttons with a clearer completion state |
| T-037 | Reset page state and cache cafe reports | feature | P1 | DONE | Codex | `/client`, `/docs/logs/2026-05.md` | Clear cafe lists on page entry, cache visitor report details by cafeId for up to 10 minutes, and show a completed state after report submission |
| T-038 | Fix reporter form reset after submit | bugfix | P1 | DONE | Codex | `/client/src/App.tsx`, `/docs/logs/2026-05.md` | Store the form element before awaiting submit so reset runs after successful report creation |
| T-039 | Rename reporter cafe list and evict stale cafe report cache | refactor | P1 | DONE | Codex | `/client/src/App.tsx`, `/docs/logs/2026-05.md` | Reporter list reads as searched cafes, and submitting a new report evicts the cached cafeId response |
| T-040 | Add reporter form validation nudges | feature | P1 | DONE | Codex | `/client/src/App.tsx`, `/docs/logs/2026-05.md` | Validate leaving time, seat count, and note length before submit and show inline nudges on invalid fields |
| T-041 | Gate reporter submit by cafe proximity | feature | P1 | DONE | Codex | `/client/src/App.tsx`, `/docs/project/architecture.md`, `/docs/project/decisions.md`, `/docs/logs/2026-05.md` | Require browser geolocation and selected cafe coordinates, and allow submission only within 100m |
| T-042 | Disable reporter inputs outside cafe radius | feature | P1 | DONE | Codex | `/client/src/App.tsx`, `/docs/logs/2026-05.md` | Lock the reporter form fields when the location check is outside the 100m radius, while keeping location re-check available |
...
| T-044 | Remove manual location check controls | refactor | P2 | IN_PROGRESS | Codex | `/client/src/App.tsx`, `/docs/logs/2026-05.md` | Hide the manual location check button and status badge now that the proximity check runs automatically on cafe selection |
